...
function WorkspaceBody({ data }: { data: ProjectWorkspaceData }) {
  ...
  return <>
    ...
    <div className="workspace-heading"><div><p className="cap">專案</p><h1>{data.project.name}</h1><MarkdownPreview value={data.project.notes ?? ''} /></div><div className="num workspace-total">{fmtHM(data.stats.totalSeconds)}<small>總工時</small></div></div>
    ...
    <section className="workspace-section"><h2>Todo</h2>{(['doing', 'todo', 'done'] as const).map((status) => <div className="workspace-task-group" key={status}><h3>{status === 'doing' ? '進行中' : status === 'todo' ? '待辦' : '已完成'} <span className="badge">{groups[status].length}</span></h3>{groups[status].map((task) => <div className="workspace-task" key={task.id}><div><strong>{task.title}</strong><MarkdownPreview value={task.notes} /></div><div className="num">{fmtHM(data.byTask.get(task.id) ?? 0)}<br /><span className="hint">{task.dueDate ?? '無期限'}</span></div></div>)}</div>)}</section>
    <section className="workspace-section"><h2>工作日誌</h2>{entries.length ? entries.map((entry) => { const task = data.tasks.find((item) => item.id === entry.taskId); return <div className="workspace-log" key={entry.id}><div className="num mute">{fmtDate(entry.startedAt)}<br />{fmtClock(entry.startedAt)}{entry.endedAt ? `–${fmtClock(entry.endedAt)}` : ''}</div><div><strong>{task?.title || entry.description || '未命名工作'}</strong><MarkdownPreview value={entry.notes} /></div><span className="num">{entry.endedAt ? fmtHM(durationSec(entry)) : '進行中'}</span></div>; }) : <div className="empty">這個專案目前沒有工作日誌</div>}</section>
