...
test('renders semantic Markdown blocks with stable paths and one editable root', () => {
  ...
  assert.equal(taskInput.dataset.markdownEditorTask, 'true');
...
test('renders table header and body cells at their containing table path', () => {
  const { root } = fixtureRoot();
  renderEditableBlocks(root, fixtureBlocks);
  ...
  assert.equal(header.dataset.blockPath, '8');
  assert.equal(header.dataset.tableAlignment, 'left');
  assert.equal(body.dataset.blockPath, '8');
  assert.equal(body.dataset.tableAlignment, 'left');
});
...
test('reads inline marks, normalizes table lines, and only retains safe links', () => {
  ...
  cell.textContent = 'line one\nline two';
  row.append(cell); head.append(row); table.append(head); root.append(table);
  assert.deepEqual(readEditableBlocks(root, [])[0], {
    type: 'table',
    header: [{ type: 'text', value: 'line one line two' }].map((inline) => [inline]),
    alignments: ['left'],
    rows: [],
  });
});
...
test('preserves the original safe Markdown URL when the browser normalizes href', () => {
  ...
  assert.equal(link.dataset.markdownUrl, 'https://example.com/a/../docs');
...
test('uses nested quote and list item paths for blocks and task checkboxes', () => {
  ...
  const taskInput = walk(root, (node) => node.tagName === 'INPUT');
  assert.equal(list.dataset.blockPath, '0.0');
  assert.equal(taskInput.dataset.markdownTaskPath, '0.0.0.0.0');
  assert.deepEqual(blockPathForNode(taskInput), [0, 0, 0, 0, 0]);
});
